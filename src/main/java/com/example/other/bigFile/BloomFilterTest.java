 ...
 * 对于一个元素，判断它是否存在于集合内，我们用不同的hash算法去算他，比如有3个，假设每一个算出来都是一个数吧，以这个数为下标，我们就把位数组里面相应的数设置为1
 * ，因为位数组只能设置为0或者1。3个hash函数算完后，我们得到三个下标，看着三个下标里面的元素是否是1，是1就在里面，如果有一个为0就不在里面。插入的方式也就是将相应的下标设置为1。
